...
**DoD:** Eine drei Jahre alte Nachricht in < 5 s finden, öffnen und die Datei daneben öffnen.

### Phase 5 – History-Backfill (M)
...
- [ ] Scheduler: Chat öffnen, „ältere Nachrichten laden" auslösen, auf Eintreffen warten, wiederholen,
      bis nichts mehr kommt oder das Chat-Limit (konfigurierbar) erreicht ist
- [ ] Drosselung: ein Chat gleichzeitig, Pausen zwischen Batches, nur bei Leerlauf und wenn das Handy online ist
- [ ] Fortschritt und Zustand in `sync_state`; fortsetzbar nach Neustart; Fehler pro Chat protokolliert
- [ ] Priorisierung: Direktchats und markierte Gruppen zuerst, Chat manuell oben anstellen; Tiefe pro Chat
      (`depth_limit_ts`, z. B. laute Gruppen nur 12 Monate), Default-Tiefe konfigurierbar
- [ ] UI: Fortschrittsanzeige gesamt und pro Chat, Pause/Stop, geschätzte Restdauer
- [ ] Empirie dokumentieren: wie weit reicht das Nachladen tatsächlich zurück (`docs/backfill-findings.md`)
