;(async () => {
  ...
  report.channel = await new Promise((res) => {
    ...
    window.addEventListener('message', (ev) => {
      if (ev.source === window && ev.data && ev.data.__watisPong) {
        clearTimeout(t)
        res('OK:' + JSON.stringify(ev.data.payload))
      }
    })
  ...
  const tryFetch = async (u) => {
    try {
      ...
      return 'THREW:' + e.name + ':' + String(e.message).slice(0, 80)
    }
  }
  ...
  try {
    ...
    report.wasmIsolated = 'COMPILED'
  } catch (e) {
    report.wasmIsolated = 'THREW:' + e.name
  }
  // 4. eval inside the ISOLATED world
  try {
    report.evalIsolated = eval('6*7')
  } catch (e) {
    report.evalIsolated = 'THREW:' + e.name
  }
