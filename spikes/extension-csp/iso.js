;(async () => {
  ...
  const report = {
    ...
    mainInlineScriptTag: el.hasAttribute('data-watis-inline-tag')
      ? 'RAN'
      : get('inline-tag') || 'BLOCKED-silently',
  ...
  try {
    ...
    report.idbFromIsolated = 'THREW:' + e.name + ':' + e.message
  }
  try {
    report.localStorageFromIsolated = JSON.stringify(Object.keys(localStorage).slice(0, 12))
  } catch (e) {
    report.localStorageFromIsolated = 'THREW:' + e.name
  }
  try {
    report.originFromIsolated = location.origin
  } catch (e) {}
