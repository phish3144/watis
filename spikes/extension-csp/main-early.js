(() => {
  ...
  try { mark('main-eval', eval('40+2')) } catch (e) { mark('main-eval', 'THREW:' + e.name) }
  try { mark('main-fn', new Function('return 7*6')()) } catch (e) { mark('main-fn', 'THREW:' + e.name) }
  try {
    const s = document.createElement('script');
    s.textContent = 'document.documentElement.setAttribute("data-watis-inline-tag","1")';
    document.documentElement.appendChild(s);
    s.remove();
    if (!document.documentElement.hasAttribute('data-watis-inline-tag')) mark('inline-tag', 'BLOCKED');
  } catch (e) { mark('inline-tag', 'THREW:' + e.name) }
